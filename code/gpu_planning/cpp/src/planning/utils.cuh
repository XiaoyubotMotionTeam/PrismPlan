...
inline void printCUDADeviceInfo() {
    ...
    for (int device = 0; device < deviceCount; ++device) {
        ...
        if (cudaDeviceGetAttribute(&memory_clock_rate_khz,
                                   cudaDevAttrMemoryClockRate,
                                   device) == cudaSuccess) {
            std::cout << "Memory Clock Rate: "
                      << memory_clock_rate_khz / 1000.0 << " MHz\n";
        }
...
namespace ppln::collision {
    ...
    // FK variant that also stores per-joint z-axis and origin for Jacobian computation
    template <typename Robot>
    __device__ __forceinline__ void fk_with_frames(
        const float *config, volatile float* sphere_pos, float *T,
        float *joint_z,  // output: 7 joints x 3 (z-axis in world frame)
        float *joint_o,  // output: 7 joints x 3 (origin in world frame)
        float *ee_pos,   // output: 3 (end-effector position)
        float *ee_rot,   // output: 9 (end-effector rotation, 3x3 row-major)
        const int tid
    );
    ...
    /* adapted from a fused-kinematics GPU kernel */
